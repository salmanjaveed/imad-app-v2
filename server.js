...
app.post(`/submit-comment/:articleName`, function (req, res) {
 ...
 app.get('/get-stats', function (req, res) {
   // make a select request
   // return a response with the results - select all articles along with the username
   pool.query('SELECT (select count(*)  from article) as articleCount, (select count(*) as userCount from "user") as userCount, (select count(*) as commentCount FROM comment) as commentCount', function (err, result) {
      if (err) {
          res.status(500).send(err.toString());
      } else {
